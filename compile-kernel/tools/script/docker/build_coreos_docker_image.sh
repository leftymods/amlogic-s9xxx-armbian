...
#
# Description: Build CoreOS docker image.
# Copyright (C) 2021~ https://github.com/leftymods/amlogic-s9xxx-armbian
#
#
# Command: ./compile-kernel/tools/script/docker/build_coreos_docker_image.sh
#
#======================================== Functions list ========================================
#
# error_msg     : Output error message and exit
# check_docker  : Check and install Docker environment
# find_coreos  : Find CoreOS rootfs file (coreos/*rootfs.tar.gz)
# build_docker  : Build the CoreOS Docker image
...
current_path="${PWD}"
coreos_path="${current_path}/coreos"
coreos_rootfs_file="*rootfs.tar.gz"
...
find_coreos() {
    cd ${current_path}
    echo -e "${STEPS} Searching for CoreOS rootfs file..."

    # Find whether the CoreOS file exists
    coreos_file_name="$(ls ${coreos_path}/${coreos_rootfs_file} 2>/dev/null | head -n 1 | awk -F "/" '{print $NF}')"
    if [[ -n "${coreos_file_name}" ]]; then
        echo -e "${INFO} CoreOS docker image file: [ ${coreos_file_name} ]."
    else
        error_msg "There is no [ ${coreos_rootfs_file} ] file in the [ ${coreos_path} ] directory."
    fi
...
build_docker() {
    cd ${current_path}
    echo -e "${STEPS} Building CoreOS Docker image..."

    # Move the docker image to the output directory
    rm -rf ${out_path} && mkdir -p ${out_path}
    cp -f ${coreos_path}/${coreos_file_name} ${out_path}/${coreos_file_name}
    ...
    sed -i "s|^ADD coreos-.*.tar.gz|ADD ${coreos_file_name}|g" ${out_path}/Dockerfile
...
export_docker() {
    ...
    local docker_image_name="coreos-docker-${docker_os}-${docker_arch}"
    ...
    rm -f ${out_path}/{${coreos_file_name},Dockerfile,docker_startup.sh}
...
echo -e "${STEPS} Welcome to the CoreOS Docker Image Builder."
