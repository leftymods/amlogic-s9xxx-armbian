...
#
# Description: Build CoreOS rootfs file.
# Copyright (C) 2021- https://github.com/leftymods/amlogic-s9xxx-armbian
#
# Optional parameters:
# -v, --VERSION_CODENAME: Set the version codename of CoreOS rootfs file, e.g., bookworm, resolute, etc.
...
# -k, --COMPILE_KERNEL:   Add coreos-kernel and uInitrd generation script, default none.
#
# Usage: ./compile-kernel/tools/script/docker/build_coreos_rootfs_file.sh -v bookworm -s true -c true -k true
#        ./compile-kernel/tools/script/docker/build_coreos_rootfs_file.sh -v bookworm
#
...
redo_rootfs() {
    echo -e "${STEPS} Rebuilding CoreOS [ ${version_codename} ] rootfs file..."

    # Searching for CoreOS image
    ...
    # Set image save name
    image_save_name="CoreOS_${image_version}-trunk_${version_codename}_${platform_type:-arm64}_${image_kernel}.img"

    # Searching for rootfs file
    rootfs_file="$(ls ${cache_path}/rootfs-*.tar.zst 2>/dev/null | head -n 1)"
    rootfs_save_name="CoreOS_${image_version}-${version_codename}_${platform_type:-arm64}_${image_kernel}_rootfs.tar.gz"
    ...
    if [[ -n "${rootfs_file}" ]]; then
        ...
        [[ "${?}" == "0" ]] && echo -e "${INFO} 02. CoreOS rootfs unpacked successfully." || error_msg "02. Failed to unpack rootfs file!"
...
EOF
        ...
        # 05. Add coreos-kernel script
        if [[ -n "${compile_kernel}" && "${compile_kernel}" =~ ^(true|yes)$ ]]; then
            down_script="https://raw.githubusercontent.com/leftymods/amlogic-s9xxx-armbian/refs/heads/main/build-coreos/coreos-files/common-files/usr/sbin/coreos-kernel"
            add_script="usr/sbin/coreos-kernel"
            ...
            [[ "${?}" == "0" ]] && {
                ...
                echo -e "${INFO} 05.01. coreos-kernel script added successfully."
            } || error_msg "05.01. Failed to add coreos-kernel script!"
...
echo "update-initramfs: CoreOS: Converting to u-boot format: ${tempname}" >&2
mkimage -A arm64 -O linux -T ramdisk -C gzip -n uInitrd -d $2 $tempname

echo "update-initramfs: CoreOS: Symlinking ${tempname} to /boot/uInitrd" >&2
...
echo "update-initramfs: CoreOS: done." >&2
...
EOF
            ...
            echo -e "${NOTE} 05. Skipping coreos-kernel script addition."
        fi

        # Compress the rootfs file
        sudo tar -czf ${rootfs_save_name} *
        sudo mv -f ${rootfs_save_name} ../
        [[ "${?}" == "0" ]] && echo -e "${INFO} 06. CoreOS rootfs created successfully." || error_msg "06. Failed to redo rootfs!"
    else
    ...
    if [[ -n "${image_file}" ]]; then
        ...
        [[ "${?}" == "0" ]] && echo -e "${INFO} 07. CoreOS image renamed successfully." || error_msg "07. Failed to rename the image!"
    else
        error_msg "07. Failed to find CoreOS image!"
    fi
    ...
    # Delete CoreOS build source codes and temporary files
    cd ${build_path}/
    sudo rm -rf $(ls . | grep -v "^output$" | xargs)
    [[ "${?}" == "0" ]] && echo -e "${INFO} 09. CoreOS source code cleaned up successfully." || error_msg "09. Failed to clean up!"

    cd ${current_path}/
    sync && sleep 3
}

echo -e "${STEPS} Starting to rebuild the CoreOS rootfs file..."
...
echo -e "${SUCCESS} CoreOS rootfs file rebuilt successfully."
