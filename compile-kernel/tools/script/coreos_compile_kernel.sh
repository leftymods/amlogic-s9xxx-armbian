...
#
# Description: Run on CoreOS, Compile the kernel.
...
# Compile toolchain download mirror, run on CoreOS
...
init_var() {
    ...
    # Get CoreOS PLATFORM value
    PLATFORM="$(cat ${ophub_release_file} 2>/dev/null | grep -E "^PLATFORM=.*" | cut -d"'" -f2)"
    [[ -n "${PLATFORM}" ]] && echo -e "${INFO} CoreOS PLATFORM: [ ${PLATFORM} ]"
}
...
toolchain_check() {
    ...
    sudo apt-get -qq install -y $(cat compile-kernel/tools/script/coreos-compile-kernel-depends)
...
generate_uinitrd() {
    ...
    # Copy /boot related files into coreos system
    [[ -d "/boot" ]] || mkdir -p /boot
    ...
    # Generate uInitrd file directly under coreos system
...
# This matches coreos-update behavior
...
if [[ "DTB_FAMILY" == "rockchip64" || "DTB_PLATFORM" == "rockchip" ]]; then
    # Create dtb symlink for rockchip platform (matches coreos-update behavior)
    cd /boot
...
loop_recompile() {
    ...
    for k in ${build_kernel[@]}; do
        ...
        echo -e "${INFO} CoreOS space usage before compilation: \n$(df -hT ${kernel_path}) \n"
        echo -e "${INFO} CoreOS memory before compilation: \n$(free -h) \n"
...
echo -e "${STEPS} Starting kernel compilation with CoreOS..."
echo -e "${INFO} The CoreOS environment: [ ${host_release} / ${arch_info} ]"

# Check script permission, supports running on CoreOS system.
[[ "$(id -u)" == "0" ]] || error_msg "Please run this script as root: [ sudo ./${0} ]"
[[ "${arch_info}" == "aarch64" ]] || error_msg "The script only supports running under CoreOS system."
...
echo -e "${STEPS} CoreOS space usage after compilation: \n$(df -hT ${kernel_path}) \n"
