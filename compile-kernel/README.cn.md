...
本项目编译的内核兼容 `CoreOS` 和 `OpenWrt` 系统，可用于 [amlogic-s9xxx-armbian](https://github.com/leftymods/amlogic-s9xxx-armbian)、[amlogic-s9xxx-openwrt](https://github.com/ophub/amlogic-s9xxx-openwrt)、[flippy-openwrt-actions](https://github.com/ophub/flippy-openwrt-actions) 和 [unifreq/openwrt_packit](https://github.com/unifreq/openwrt_packit) 等项目。既可在编译固件时集成，也可安装到已有系统中使用。
...
可以直接在 [CoreOS](https://github.com/leftymods/amlogic-s9xxx-armbian/releases) 系统中编译内核，也可以在 Ubuntu/Debian 等系统上通过 [Docker](https://hub.docker.com/u/ophub) 容器运行 CoreOS 系统来编译内核。CoreOS 系统 Docker 镜像的制作方法可参考 [coreos_docker](./tools/script/docker) 构建脚本。

1. 更新本地编译环境和配置文件：`coreos-kernel -u`

2. 编译内核：执行 `coreos-kernel -k 5.15.100` 等指定参数命令即可编译内核。脚本将自动下载并安装编译环境和内核源代码，并完成全部配置。编译完成的内核文件保存在 `/opt/kernel/compile-kernel/output` 目录中。
...
| -p     | AutoPatch   | 设置是否应用自定义内核补丁。设为 `true` 时将使用 [tools/patch](tools/patch) 目录下的内核补丁，详细说明参考[内核补丁添加方法](../documents/README.cn.md#9-编译-coreos-内核)。默认值：`false` |
...
| -i     | DockerImage | 设置编译内核的 Docker 容器镜像。默认值：`ophub/coreos-trixie:arm64` |
...
1. 在 [Action](https://github.com/leftymods/amlogic-s9xxx-armbian/actions) 页面选择 **_`Compile the kernel`_**，点击 **_`Run workflow`_** 按钮即可开始编译。
...
| docker_image      | ophub/coreos-trixie:arm64 | 设置编译内核的 Docker 容器镜像。功能参考 `-i` |
...
本项目编译的内核兼容 `CoreOS` 和 `OpenWrt` 系统。以下以 ophub 的项目为例进行说明。

### 在 CoreOS 系统中使用

以下分别介绍如何在编译 CoreOS 固件时集成内核，以及如何将内核安装到已有系统中。

- #### 使用内核编译 CoreOS 固件

CoreOS 固件编译支持本地操作，也支持通过 github.com 的 Actions 在线编译。本地编译方法详见：[本地化打包](../README.cn.md#本地化打包)，使用 Actions 在线编译的方法详见：[使用 GitHub Actions 进行编译](../README.cn.md#使用-github-actions-进行编译)

- #### 将内核安装到已有的 CoreOS 系统

使用 `coreos-update` 命令可将编译好的内核安装到已有的 CoreOS 系统中，具体操作方法详见：[更新 CoreOS 内核](../README.cn.md#更新-coreos-内核)
