...
# 4) /boot/coreosEnv.txt : fdtfile=vendor/xxx.dtb  (or fdtfile=xxx.dtb)
[[ -z "${FDTFILE}" && -f "/boot/coreosEnv.txt" ]] &&
    FDTFILE="$(grep -E '^fdtfile=.*\.dtb$' /boot/coreosEnv.txt 2>/dev/null | head -n1 | sed -E 's#^fdtfile=##; s#.*/##')"
...
if [[ "${openvfd_boxid}" != "0" && "${FDTFILE}" =~ ^meson- ]]; then
    (
        ...
        [[ "${openvfd_enable}" == "yes" ]] && coreos-openvfd "${openvfd_boxid}" >/dev/null 2>&1
        # Some devices require a restart to clear 'BOOT' and related messages
        [[ "${openvfd_restart}" == "yes" ]] && {
            coreos-openvfd "0" >/dev/null 2>&1
            sleep 3
            coreos-openvfd "${openvfd_boxid}" >/dev/null 2>&1
        }
